...
Apache Kafka was open-sourced by LinkedIn in early 2011. Despite all the initial
...
few years but the original gotchas remain:
...
Kafka doesn't like big messages. By default, it's limited to 1Mb. You can modify
...
without modifying
[message.max.bytes](https://kafka.apache.org/documentation/#brokerconfigs_message.max.bytes).
Let's review three different options:
...
1. **Split the message into several messages**: this is possible almost always
   ...
   idea) so duplicates don't have a big impact on the overall size of the batch.

2. **Move messages to different storage**: when messages are too big and there
   ...
   [some libraries](https://github.com/bakdata/kafka-s3-backed-serde) that allow
...
If anything of that works, then go and increase
[message.max.bytes](https://kafka.apache.org/documentation/#brokerconfigs_message.max.bytes).
...
1. **Consumers interested in some part of the data have to read everything** and
   ...
   consumer has to read them and consume network bandwidth anyway.
2. **A big topic usually means a lot of partitions**. It's how Kafka works: a
...
3. **Big topics mean big clusters**. We'll cover big clusters later but it's
   ...
   impossible.
4. There are several operations in Kafka that require
   **creating a new version of the topic**: typically decrease the number of
   ...
   much simpler because there are fewer consumers impacted.
...
pattern so topics will be assigned dynamically even if they are created later.

## Small clusters

Big clusters are hard to maintain even for the companies with the most Kafka
experience. You can see in
[this thread](https://twitter.com/gunnarmorling/status/1384903606407729153) many
...
about it (Kafka deployment strategy section).

Another problem for big clusters is
**configurations that are done at cluster-level** (
[message.max.bytes](https://kafka.apache.org/documentation/#brokerconfigs_message.max.bytes),
...
optimize them for particular loads.
...
split, they are easy to manage and you can control producers and consumers.
...
much) data between clusters.

## Summary

This is a list of only three things (but there are much more!) that may help to
...
by being aware of the challenges and plan for the consequences.

Did I miss something? You can comment on
[GitHub](https://github.com/antonmry/galiglobal/pull/39) or just drop me a note
