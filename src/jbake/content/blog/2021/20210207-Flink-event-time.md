...
different sensors. Every sensor will send measures for each 100ms.  We would
...
- WatermarkStrategy.[forBoundedOutOfOrderness]: this is a periodic generator
  that allows dealing with records out of order when it's inside a defined
  range.
- WatermarkStrategy.[forMonotonousTimestamps]: this is the same as
...
for that it's because the job is ending before any watermark is generated.
We could set the periodic generation with a smaller value but the problem
...
[Apache Flink] is a great framework and it supports Event time in a nice way.
The concept of watermarks as events in the pipeline is superb and full of
